...
$ids = required_param_array('ids', PARAM_TEXT);
...
foreach ($destination as $i => $x) {
        ...
        $touser->username = $DB->get_field('user', 'username', array('id' => $destination[$i]));
...
if ($ccteachers) {
    ...
    foreach ($userstocopyemail as $i) {
        $touser->id = $i->id;
        $touser->username = $DB->get_field('user', 'username', array('id' => $i->id));
